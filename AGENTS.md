...
> ⚠️ **凡需要用户确认 / 选择 / 补充信息的环节，必须调用 `ask_user_question` 弹窗工具，禁止在正文里写"回复 ok""请确认"等纯文本等待。**

| 场景 | 要求 |
|------|------|
| 需要用户做决定（确认、选择方案、处理冲突） | 弹窗，2–4 个选项 |
| 选项呈现 | 推荐项放第一位，标签末尾标注"（推荐）"，`description` 说明取舍与影响 |
| 纯告知性内容（进度、复核报告、失败原因） | 正文输出，不弹窗 |
| 用户明确说"你自己决定" | 采用推荐项，并在正文说明所选方案 |

**理由**：正文提问容易被忽略或漏答，弹窗让决策点显式、可点选，避免流程停在"等回复"上。发布/立项等技能已按此规则改写。
