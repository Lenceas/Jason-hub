...
> Jason-hub 发布流程总览。**详细步骤已封装为技能**（`.dsh/skills/`），执行时直接调用对应命令或说对应口令即可。
>
> 🎯 **快捷命令**（无需预加载上下文）：
>   - `/release` — 主仓库发布流（7 步 + 子项目分支同步）
>   - `/project-release` — 子项目发布流
>   - `/project-init` — 子项目立项流
>   - `/scaffold-dotnet` — 创建 .NET 后端子项目
>   - `/load-project` — 加载项目全景
>
> DSH 会话中既可直接说口令（如"提交代码"），也可用上面的斜杠命令，两者等价。
...
开发完成 → /project-release
    ...
    ├──────────────────────┤
    │ ③ 镜像构建判断        │ ← 代码变更→ docker build + push TCR
    ├──────────────────────┤
    │ ④ 识别变更范围        │
    ├──────────────────────┤
    │ ⑤ 更新 MD 文档        │ ← 按清单逐条执行
