...
| 2 | `{子项目}/-Changelog.md` | 子项目有改动 | 检测 | 子项目级版本日志同步 |
| 3 | `README.md`（根） | 新增子项目/结构/域名/在线地址变动 | 检测 | 目录结构图、子项目表、在线地址、快速开始 |
| 4 | `{子项目}/README.md` | 子项目组件/技术栈/配置变动 | 检测 | 组件列表、目录结构、技术栈表、开发命令 |
| 5 | `DEPLOY.md` | 部署流程/端口/域名/Nginx/SSL/Docker 变动 | 检测 | 全文档审阅，与实际配置不一致则更新 |
| 6 | `ARCHITECTURE.md` | 架构层次/部署拓扑/技术选型变动 | 检测 | 架构图、分层描述、技术选型表 |
| 7 | `AGENTS.md` | 技术栈/目录结构/端口分配/域名规则变动 | 检测 | 端口分配表、技术栈表、目录结构 |
| 8 | `STYLE_GUIDE.md` | 命名规范/分支策略/提交格式变动 | 检测 | 全文审阅 |
| 9 | `RELEASE.md` | 发布流程本身有变动 | 检测 | 本文件 |
