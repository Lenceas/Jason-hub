...
> ⚠️ **本项目所有需要用户确认 / 选择 / 补充信息的环节，一律调用 `ask_user_question` 弹窗工具，禁止在正文里写"回复 ok""请确认"等纯文本等待。**

| 环节 | 做法 |
|------|------|
| 步骤③ 是否构建镜像 | 弹窗，选项：不构建（推荐）/ 构建并推送 |
| 步骤⑥ 改动摘要确认 | 弹窗，选项：确认执行 / 需要调整 |
| 发现无关改动如何处置（如 stash） | 弹窗给方案选项 |
| 发布范围、版本位存在歧义时 | 弹窗给候选方案 |
| 敏感信息/规范冲突等需要决策时 | 弹窗给处置选项 |

- 每个问题给 **2–4 个选项**，把推荐项放第一个并在标签末尾标注"（推荐）"，用 `description` 说明取舍与影响。
- 纯告知性内容（进度、复核报告、失败原因）仍用正文输出，不弹窗——**弹窗只用于需要用户决策的点**。
- 弹窗返回后按其结果继续，不得因未收到正文回复而停滞。
- 用户明确说"你自己决定"时，采用推荐项并在正文说明所选方案。
...
**必须用 `ask_user_question` 弹窗确认：本次变更是否需要构建 Docker 镜像？**（选项：不构建（推荐，仅文档/规范变更时）/ 构建并推送镜像）
...
**必须用 `ask_user_question` 弹窗确认**（选项："确认执行" / "需要调整"），收到确认后才进入步骤⑦。禁止用正文"回复 ok"等待。
